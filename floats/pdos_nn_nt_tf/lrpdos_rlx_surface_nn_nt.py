...
data_Nb = [np.load('nn_total_rlx.npz'), np.load('Nb_d_separated_1_rlx.npz')]
data_Ta = [np.load('nt_total_rlx.npz'), np.load('Ta_d_separated_1_rlx.npz')]
output_file = 'pdos_rlx_nn_nt_surface'
xlim = (0, 9)
ylim_Nb = (-2.5, 2.5)
ylim_Ta = (-0.5, 0.5)
width = 0.5*((160 - 15) - 10)
...
###############################################################################
    ...
    for orb in range(len(d_colors)):
        axs[1].plot(
            ...
            x_axis_Ta, -data_Ta[1]['pdos'][1, :,4 + orb],
            linewidth = 1.0,
            color = d_colors[orb]
        )
